...
<body class="antialiased text-slate-800">
    ...
    <div id="app" class="container mx-auto p-4 md:p-8 max-w-5xl">
        <header class="flex justify-between items-center mb-6 pb-4 border-b border-slate-300">
            ...
            <div class="flex gap-2">
                ...
                <button id="save-btn" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-5 rounded-lg shadow-md transition-transform transform hover:scale-105">
        ...
        <div id="notation-container" class="bg-white p-4 rounded-lg shadow-inner border border-slate-200 mb-6 overflow-x-auto min-h-[150px]">
            <div id="lines-container"></div>
            <button id="add-line-btn" class="mt-4 bg-slate-200 hover:bg-slate-300 text-slate-700 font-semibold py-2 px-4 rounded-lg text-sm">
    ...
    <div id="virtual-keyboard-container" class="bg-slate-100 md:bg-white md:static md:p-4 md:rounded-lg md:shadow-md md:border md:border-slate-200">
        <div class="flex border-b border-slate-200 mb-3">
            ...
            <button id="toggle-lyrics" class="keyboard-toggle flex-1 py-3 px-4 font-semibold text-slate-500">Keyboard Lirik</button>
            <button id="hide-keyboard-btn" class="md:hidden p-3 text-slate-600"><i class="fas fa-chevron-down"></i></button>
        </div>
        <div id="keyboard-notes" class="p-2 grid grid-cols-7 sm:grid-cols-9 md:grid-cols-11 gap-1.5"></div>
        <div id="keyboard-lyrics" class="p-2 hidden"></div>
    </div>
    ...
    <script>
        window.onload = function() {
            ...
            const lyricKeys = ['QWERTYUIOP'.split(''), 'ASDFGHJKL'.split(''), ['Shift', ...'ZXCVBNM'.split(''), 'Bksp'],
                ['Space']
            ];
            ...
            // ==========================================================
            // === FUNGSI RENDER GRID DIPERBARUI UNTUK FOKUS OTOMATIS ===
            // ==========================================================
            function renderNotationGrid(focusTarget = null) {
                ...
                // Logika untuk fokus ke elemen baru setelah render selesai
                if (focusTarget) {
            ...
            function generateKeyboards() {
                ...
                noteKeys.forEach(key => {
                    ...
                    keyBtn.className = 'keyboard-key h-12 rounded-lg shadow font-semibold transition transform hover:scale-105 ' + ((key === 'Del') ? 'bg-red-200 text-red-800' : 'bg-slate-200');
                ...
                lyricKeys.forEach(row => {
                    ...
                    rowDiv.className = 'flex justify-center space-x-1.5';
                    row.forEach(key => {
                        const keyBtn = document.createElement('button');
                        keyBtn.textContent = key;
                        keyBtn.dataset.key = key;
            ...
            // --- EVENT LISTENER DIPERBARUI UNTUK FOKUS ---
            ...
            function handleKeyboardInput(key) {
                ...
                if (type === 'note') {
                    ...
                    else if (key !== 'Shift') activeInput.value += key;
                }
                const {
                    line,
                    slot
            ...
            [dom.keyboardNotes, dom.keyboardLyrics].forEach(kb => {
